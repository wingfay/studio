...
namespace APIPro
{
   public partial class frmAPIPro : Form
   {
      ...
      [DllImport("User32.dll", SetLastError = true, CharSet = CharSet.Auto)]
      static extern long GetWindowTextA(IntPtr hwnd, StringBuilder lpString, long cch);
      ...
      private void tmrCursorPos_Tick(object sender, EventArgs e)
      {
         ...
         if (retVal)
         {
            ...
            if (hwnd.ToInt64() > 0)
            {
               ...
               if(rtbClassName.Text.ToUpper()== "NetUIHWND")
					{
                  var icon = IconHelper.GetAppIcon(hwnd);

						if (icon != null)
						{
                     pictureBox1.Image = null;
                     pictureBox1.Image = IconHelper.ImageFromIcon(icon);
						}
					}
               rtbText.Text = GetControlText(hwnd);

               //For Parent
               IntPtr hWndParent = GetParent(hwnd);
               if (hWndParent.ToInt64() > 0)
               {
                  rtbWinParent.Text = GetClassNameOfWindow(hWndParent); //GetCaptionOfWindow(hWndParent);
               }
            }
